...
import com.zxd.blackt.blackt.Entity.Songg;
...
/**
 * Created by zhuangxd on 2017/7/3.
 */
...
public class LRCView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        if (mSong != null && mSong.slrc != null && mSong.slrc.size() > 0) {
            for (int i = 0, size = mLineCount; i < size; i++) {
                ...
                canvas.drawText(mSong.slrc.get(i).lcontent, x, y, mTextPaint);
            }
        } else {
    ...
    private String measureCurrentTime() {
        ...
        if (mSong != null && mLineCount > 0 && mCurrentShowLine - 1 < mLineCount && mCurrentShowLine > 0) {
            return format.format(mSong.slrc.get(mCurrentShowLine - 1).ltime / 1000 / 60) + ":" + format.format(mSong.slrc.get(mCurrentShowLine - 1).ltime / 1000 % 60);
        }
        if (mSong != null && mLineCount > 0 && (mCurrentShowLine - 1) >= mLineCount) {
            return format.format(mSong.slrc.get(mLineCount - 1).ltime / 1000 / 60) + ":" + format.format(mSong.slrc.get(mLineCount - 1).ltime / 1000 % 60);
        }
        if (mSong != null && mLineCount > 0 && mCurrentShowLine - 1 <= 0) {
            return format.format(mSong.slrc.get(0).ltime / 1000 / 60) + ":" + format.format(mSong.slrc.get(0).ltime / 1000 % 60);
        }
        return mDefaultTime;
    }
            ...
            if (mIndicatorShow && clickPlayer(event)) {
                if (mCurrentShowLine != mCurrentPlayLine) {
                    ...
                    if (mClickListener != null) {
                        mClickListener.onPlayerClicked(mSong.slrc.get(mCurrentShowLine - 1).ltime, mSong.slrc.get(mCurrentShowLine - 1).lcontent);
                    }
                }
            }
        }
    ...
    private boolean scrollable() {
        return mSong != null && mSong.slrc != null && mSong.slrc.size() > 0;
    }
    ...
    private void scrollToCurrentTimeMillis(long time) {
        ...
        if (scrollable()) {
            for (int i = 0, size = mLineCount; i < size; i++) {
                Songg songg = mSong.slrc.get(i);
                if (songg != null && songg.ltime > time) {
                    position = i;
                    break;
                }
                if (i == mLineCount - 1) {
     ...
     */
    private void readLRC(InputStream inputStream, String charsetName) {
        try {
            Log.d("-------->>", "读文件");
            Song song = new Song();
            song.slrc = new ArrayList<>();
     ...
     * 逐行解析歌词内容
     */
    private void parselrc(Song song, String line) {
        Log.d("-------->>", "解析文件");
        String relrc = line.replace("[", "@");
        String[] strs_1 = relrc.split("@");
        List<String> strlist = new ArrayList<String>();
        for (int i = 0; i < strs_1.length; i++) {
            strlist.add(strs_1[i]);
        }
        if (strlist != null) {
            for (String s : strlist) {
                if (s.contains("ti:")) {
                    String t = s.replace("ti:", "");
                    String tt = t.replace("]", "");
                    song.ti = tt;
                } else if (s.contains("ar:")) {
                    String t = s.replace("ar:", "");
                    String tt = t.replace("]", "");
                    song.ar = tt;
                } else if (s.contains("al:")) {
                    String t = s.replace("al:", "");
                    String tt = t.replace("]", "");
                    song.al = tt;
                } else if (s.contains("by:")) {
                    String t = s.replace("by:", "");
                    String tt = t.replace("]", "");
                    song.by = tt;
                } else if (s.contains("offset:")) {
                    String t = s.replace("offset:", "");
                    String tt = t.replace("]", "");
                    song.offset = Long.parseLong(tt);
                } else {
                    int index = s.lastIndexOf("]");
                    if (index != -1) {
                        if (s != null && index == 8 && s.trim().length() > 10) {
                            String ss = s.replace(".", ":");
                            songg = new Songg();
                            songg.lcontent = s.substring(9, ss.length());
                            songg.ltime = measureStartTimeMillis(ss.substring(0, 8));
                            Log.d("-------->>", "解析文件：" + songg.lcontent + "；" + songg.ltime + ";" + ss.substring(0, 8));
                            song.slrc.add(songg);
                        }
                    }
                }
    ...
    private long measureStartTimeMillis(String str) {
        long min = Long.parseLong(str.substring(0, 2));
        long sec = Long.parseLong(str.substring(3, 5));
//        long ms = Long.parseLong(str.substring(6, 8)); + ms * 60 * 1000
     ...
     * @param current 时间戳
     */
    public void setCurrentTimeMillis(String current) {
        long currentt = measureStartTimeMillis(current);
        Log.d("------->", "当前时间：" + currentt);
        scrollToCurrentTimeMillis(currentt);
    }
    ...
    public void setLyricFile(File file, String charsetName) {
        if (file != null && file.exists()) {
            try {
                readLRC(new FileInputStream(file), charsetName);
            } catch (Exception e) {
                e.printStackTrace();
            }
        } else {
